...
node ("ubuntu1404") {
    ...
    stage ("Checkout") { 
	    ...
	    step([$class: 'CopyArtifact', filter: '**/*.zip', fingerprintArtifacts: true, flatten: true, projectName: 'Tanium/Content-BuildTools/master', selector: [$class: 'SavedBuildSelector'], target: 'tools'])
	...
	env.INTERNAL_BRANCHES_TO_PUBLISH = ""
	buildfile.buildAll()
